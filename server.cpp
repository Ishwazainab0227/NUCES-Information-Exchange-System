...
void printLine(const string& color = CYAN, char ch = '=', int length = 80) {
    std::cout << color << string(length, ch) << RESET << std::endl;
}
void printHeader(const string& text, const string& color = BRIGHT_CYAN) {
    int padding = (80 - static_cast<int>(text.length())) / 2;
    printLine(color, '=', 80);
    std::cout << color << "|" << string(max(0, padding - 1), ' ') << BOLD << text 
              << RESET << color << string(max(0, 80 - padding - static_cast<int>(text.length()) - 1), ' ') << "|" << RESET << std::endl;
    printLine(color, '=', 80);
}
...
void printLog(const string& message, const string& type = "INFO") {
    ...
    std::cout << DIM << "[" << BRIGHT_WHITE << getCurrentTime() << DIM << "]" << RESET 
              << " " << typeColor << icon << " " << BOLD << setw(12) << left << type << RESET 
              << " " << WHITE << message << RESET << std::endl;
}
...
void handleCampusClient(SOCKET clientSocket, sockaddr_in clientAddr) {
    ...
    while (true) {
        ...
        if (message.find("FILE_TRANSFER:") == 0) {
            ...
            size_t filenamePos = message.find("|FILENAME:");
            size_t sizePos = message.find("|SIZE:");
            size_t dataPos = message.find("|DATA:");
            
            if (deptPos != string::npos && fromPos != string::npos && 
                filenamePos != string::npos && sizePos != string::npos && 
                dataPos != string::npos) {
                
                string targetCampus = message.substr(14, deptPos - 14); // 14 = length of "FILE_TRANSFER:"
                string targetDept = message.substr(deptPos + 6, fromPos - deptPos - 6);
                string sourceCampus = message.substr(fromPos + 6, filenamePos - fromPos - 6);
                string filename = message.substr(filenamePos + 10, sizePos - filenamePos - 10);
                string sizeStr = message.substr(sizePos + 6, dataPos - sizePos - 6);
                
                printLog(string(CYAN) + sourceCampus + RESET + " -> " + YELLOW + targetCampus + RESET + 
                        " [FILE: " + GREEN + filename + RESET + " - " + sizeStr + " bytes]", "FILE");
                
                // Forward file transfer to target campus
                if (sendToClient(targetCampus, message)) {
                    string ack = "ACK:File transferred to " + targetCampus;
                    send(clientSocket, ack.c_str(), static_cast<int>(ack.length()), 0);
                } else {
                    string error = "ERROR:Unable to transfer file to " + targetCampus;
                    send(clientSocket, error.c_str(), static_cast<int>(error.length()), 0);
                    printLog("Failed to transfer file to: " + targetCampus, "ERROR");
                }
            }
        }
...
void adminModule(SOCKET udpSocket) {
    ...
    while (true) {
        ...
        std::cout << BRIGHT_YELLOW << "+" << string(78, '-') << "+" << RESET << std::endl;
        std::cout << BRIGHT_YELLOW << "|" << BOLD << setw(35) << right << "" 
                  << "ADMIN CONSOLE" << setw(31) << "" << RESET << BRIGHT_YELLOW << "|" << RESET << std::endl;
        std::cout << BRIGHT_YELLOW << "+" << string(78, '-') << "+" << RESET << std::endl;
        printLine(BRIGHT_YELLOW, '-', 80);
        
        std::cout << BRIGHT_CYAN << "  [1]" << RESET << " View Connected Campuses" << std::endl;
        std::cout << BRIGHT_CYAN << "  [2]" << RESET << " Broadcast Announcement" << std::endl;
        std::cout << BRIGHT_CYAN << "  [3]" << RESET << " Exit Admin" << std::endl;
        printLine(BRIGHT_YELLOW, '-', 80);
        std::cout << BRIGHT_WHITE << ">> Choice: " << RESET;
        ...
        if (input == "1") {
            ...
            if (connectedClients.empty()) {
                ...
                std::cout << "\n";
                std::cout << BOLD << "  " << setw(20) << left << "CAMPUS" 
                         << setw(25) << "LAST HEARTBEAT" 
                         << setw(15) << "STATUS" << RESET << std::endl;
                printLine(CYAN, '-', 80);
            ...
            std::cout << "\n";
            printLine(YELLOW, '-', 80);
            std::cout << BRIGHT_YELLOW << "[B] Enter announcement message:\n" << RESET;
...
int main() {
    ...
    if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
        std::cerr << RED << "[X] WSAStartup failed" << RESET << std::endl;
        return 1;
    }
    #endif
    enableANSI();
    ...
    std::cout << BRIGHT_CYAN << "|" << string(78, ' ') << "|" << RESET << std::endl;
    std::cout << BRIGHT_CYAN << "|" << BRIGHT_WHITE << BOLD << setw(45) << right 
              << "NU-INFORMATION EXCHANGE SYSTEM" << setw(34) << "" << RESET 
              << BRIGHT_CYAN << "|" << RESET << std::endl;
    std::cout << BRIGHT_CYAN << "|" << CYAN << setw(47) << right 
    ...
    std::cout << BRIGHT_YELLOW << "  Press Enter to start admin console..." << RESET << std::flush;
    string dummy;
