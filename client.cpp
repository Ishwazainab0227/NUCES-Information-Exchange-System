...
bool readFileContent(const string& filepath, string& content, string& filename) {
...
bool saveFile(const string& filename, const string& content) {
    ...
    printLog("File saved as: " + filepath);
    return true;
}
...
void listenForBroadcasts() {
    ...
    printLog("Listening for broadcasts on port " + to_string(clientUdpPort));
...
void listenForMessages(SOCKET tcpSocket) {
    ...
    while (isRunning) {
        ...
        // FILE TRANSFER
        else if (message.rfind("FILE_TRANSFER:", 0) == 0) {
            ...
            if (deptPos != string::npos && fromPos != string::npos && 
                ...
                string fileData = message.substr(dataPos + 6);
                ...
                if (saveFile(filename, fileData)) {
                    ...
                    cout << "\n" << GREEN << BOLD
                         ...
                         << RESET << "\n" << endl;
                    cout << CYAN << "File: " << filename << RESET << endl;
                    cout << CYAN << "From: " << sourceCampus << RESET << "\n" << endl;
                }
            }
        }
...
void displayMenu(SOCKET tcpSocket, const string& campusName) {
    ...
    while (isRunning) {
        ...
        cout << GREEN << "2. Send File to Another Campus" << RESET << endl;
        cout << GREEN << "3. View Received Messages" << RESET << endl;
        ...
        if (input == "1") {
            ...
            // FILE TRANSFER
            string targetCampus, targetDept, filepath;
            ...
            cout << WHITE << BOLD << "Enter file path: " << RESET;
            getline(cin, filepath);
            
            if (readFileContent(filepath, fileContent, filename)) {
                // Format: FILE_TRANSFER:CampusName|DEPT:DeptName|FROM:SourceCampus|FILENAME:name|SIZE:bytes|DATA:content
                string formattedMsg = "FILE_TRANSFER:" + targetCampus +
                                      "|DEPT:" + targetDept +
                                      "|FROM:" + campusName +
                                      "|FILENAME:" + filename +
                                      "|SIZE:" + to_string(fileContent.size()) +
                                      "|DATA:" + fileContent;
                
                int result = send(tcpSocket, formattedMsg.c_str(), formattedMsg.length(), 0);
                
                if (result == SOCKET_ERROR) {
                    cout << RED << BOLD << "ERROR - Unable to transfer file" << RESET << endl;
                } else {
                    cout << GREEN << BOLD << "File sent successfully!" << RESET << endl;
                    cout << CYAN << "File: " << filename << " (" << fileContent.size() << " bytes)" << RESET << endl;
                }
            } else {
                cout << RED << BOLD << "ERROR - Could not read file: " << filepath << RESET << endl;
            }
            waitAndClear();
